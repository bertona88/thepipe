use pipe_sim::handoff::{controller::{Action, Phase}, run_stationary_coupon, Fault};

#[test]
fn stationary_exchange_has_one_owner_and_no_pose_jump() {
    let report = run_stationary_coupon(Fault::None).unwrap();
    assert_eq!(report.status, Phase::Complete, "{:?}", report.terminal_reason);
    ...
    assert_eq!(actions, vec![Action::AcquireDonor, Action::CloseReceiver,
        Action::TransferOwnership, Action::OpenDonor, Action::VerifyReceiver]);
    for frame in &report.evaluation_only_frames {
        let truth=frame.truth.as_ref().unwrap();
        assert!(truth.manipulators.iter().filter(|arm| arm.gripper.held_body_id.is_some()).count() <= 1);
    }
...
fn observation_loss_never_releases_the_current_owner() {
    for (fault,owner,reason) in [
        (Fault::ObservationLostBeforeClose,1,"observation_unavailable"),
        (Fault::ObservationLostBeforeTransfer,1,"observation_unavailable"),
        (Fault::ObservationLostAfterTransfer,2,"observation_unavailable"),
        ...
        assert_eq!(report.evaluation_only_maximum_interarm_penetration_m,0.0);
        assert!(report.evaluation_only_frames.last().unwrap().commanded.manipulators.iter().all(|arm| arm.stopped));
        assert!(!report.decisions.iter().any(|d| d.action==Action::OpenDonor));
    }
}
