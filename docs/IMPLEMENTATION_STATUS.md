...
| Machine runtime | Canonical hashed SI machine configuration; explicit paired-belt/end-bogie topology; bounded and sequenced carriage, joint, gripper, stop, and Cartesian point commands; carriage-first tool-position IK; synchronized limit-derived trajectories; sampled collision preflight; numeric TCP error and deterministic replay; named FK frames; stable IDs; link collision capsules | Constrained-orientation IK, continuous/obstacle-avoiding path planning, carriage optimization, watchdog/safety state machine, identified rail errors, and a hardware backend |
